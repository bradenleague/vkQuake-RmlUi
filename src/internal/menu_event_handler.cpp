...
void MenuEventHandler::ProcessEvent (Rml::Event &event)
{
	...
	Con_DPrintf (
		"MenuEventHandler: event=%s action=%s target=%s id=%s\n", event.GetType ().c_str (), action.c_str (), element->GetTagName ().c_str (),
		element->GetId ().c_str ());
...
void MenuEventHandler::ExecuteAction (const std::string &action)
{
	...
	std::string func_name = (paren_pos != std::string::npos) ? action.substr (0, paren_pos) : action;
...
std::pair<std::string, int> MenuEventHandler::ExtractTwoArgs (const std::string &action)
{
	...
	if (comma == std::string::npos)
	{
		...
		if (arg1.length () >= 2 && ((arg1.front () == '\'' && arg1.back () == '\'') || (arg1.front () == '"' && arg1.back () == '"')))
		{
...
void MenuEventHandler::ActionNavigate (const std::string &menu_path)
{
	...
	if (menu_path == "load_game" || menu_path == "save_game" || full_path.find ("load_game") != std::string::npos ||
		full_path.find ("save_game") != std::string::npos)
	{
		M_SyncSavesToUI ();
	}

	if (menu_path == "options_video" || full_path.find ("options_video") != std::string::npos)
	{
		VID_SyncModesToUI ();
	}

	if (menu_path == "mods" || full_path.find ("mods") != std::string::npos)
	{
		M_SyncModsToUI ();
	}

	if (menu_path == "options_keys" || full_path.find ("options_keys") != std::string::npos)
	{
