...
bool GameDataModel::Initialize(Rml::Context* context)
{
    ...
    constructor.BindFunc("game_title",
        [](Rml::Variant& variant) {
    ...
    constructor.BindFunc("time_seconds",
        [](Rml::Variant& variant) {
            char buf[4];
    ...
    constructor.BindFunc("weapon_label",
        [](Rml::Variant& variant) {
            switch (g_game_state.active_weapon) {
                case IT_AXE: variant = Rml::String("AXE"); break;
    ...
    constructor.BindFunc("is_axe",
        [](Rml::Variant& variant) {
            variant = (g_game_state.active_weapon == IT_AXE);
        });
    ...
    constructor.BindFunc("chat_active",
        [](Rml::Variant& variant) {
            variant = (key_dest == key_message);
        });
    constructor.BindFunc("chat_prefix",
        [](Rml::Variant& variant) {
            variant = Rml::String(chat_team ? "say_team:" : "say:");
        });
    constructor.BindFunc("chat_text",
        [](Rml::Variant& variant) {
    ...
    constructor.BindEventCallback("load_slot",
    ...
    constructor.BindEventCallback("save_slot",
        [](Rml::DataModelHandle, Rml::Event& event, const Rml::VariantList& arguments) {
            if (arguments.empty()) return;
            Rml::String slot_id = arguments[0].Get<Rml::String>();
            if (slot_id.empty()) return;
    ...
    constructor.BindEventCallback("select_mod",
        [](Rml::DataModelHandle, Rml::Event& event, const Rml::VariantList& arguments) {
            if (arguments.empty()) return;
...
void GameDataModel::Update()
{
    ...
    if (g_game_state.num_players != s_prev_state.num_players ||
        g_game_state.players != s_prev_state.players) {
                                  ...
                                  double game_time)
{
    ...
    if (!stats || stats_count < STAT_ITEMS + 1) return;
