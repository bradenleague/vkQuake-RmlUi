...
void CvarBindingManager::RegisterAllBindings()
{
    ...
    // ── Computed labels for video mode picker (BindFunc, not backed by cvar) ──
    {
        ...
        if (constructor) {
            constructor.BindFunc("vid_mode_label",
                [](Rml::Variant& variant) {
            ...
            constructor.BindFunc("vid_rate_label",
                [](Rml::Variant& variant) {
                    int r = static_cast<int>(GetProvider()->GetFloat("vid_refreshrate"));
                    variant = Rml::String(std::to_string(r) + " Hz");
                });
            constructor.BindFunc("vid_fullscreen_label",
                [](Rml::Variant& variant) {
                    int fs = static_cast<int>(GetProvider()->GetFloat("vid_fullscreen"));
                    switch (fs) {
                        case 0: variant = Rml::String("Windowed"); break;
                        case 1: variant = Rml::String("Fullscreen"); break;
                        case 2: variant = Rml::String("Exclusive"); break;
                        default: variant = Rml::String("Unknown"); break;
                    }
                });
            constructor.BindFunc("vid_vsync_label",
                [](Rml::Variant& variant) {
                    int vs = static_cast<int>(GetProvider()->GetFloat("vid_vsync"));
                    switch (vs) {
                        case 0: variant = Rml::String("Off"); break;
                        case 1: variant = Rml::String("On"); break;
                        case 2: variant = Rml::String("Triple Buffer"); break;
                        default: variant = Rml::String("Unknown"); break;
                    }
                });
        }
    ...
    // ── Keybind data model for controls menu ──
    {
        ...
        if (constructor) {
            ...
            constructor.BindEventCallback("capture_key",
    ...
    // ── Video mode list for resolution picker ──
    {
        ...
        if (constructor) {
            ...
            constructor.BindEventCallback("select_resolution",
