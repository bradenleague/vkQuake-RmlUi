...
void UpdateDpRatio ()
{
	if (!g_state.context || g_state.width <= 0 || g_state.height <= 0)
		return;
...
extern "C"
{
	...
	void UI_SetInputMode (ui_input_mode_t mode)
	{
		...
		Con_DPrintf (
			...
			: old_mode == UI_INPUT_MENU_ACTIVE ? "MENU_ACTIVE"
											   : "OVERLAY",
			mode == UI_INPUT_INACTIVE	   ? "INACTIVE"
			: mode == UI_INPUT_MENU_ACTIVE ? "MENU_ACTIVE"
										   : "OVERLAY");
	}
		...
		double game_time)
	{
		...
		GameDataModel_SyncFromQuake (stats, stats_count, items, intermission, gametype, maxclients, level_name, map_name, game_time);
	}
